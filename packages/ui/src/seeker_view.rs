use api::models::{AddZoneRequest, ExclusionZone, MapDetail, MapQuestion, SessionInfo};
use dioxus::prelude::*;
use uuid::Uuid;

#[component]
pub fn SeekerViewComponent(
    game_id: Uuid,
    session: SessionInfo,
    map: MapDetail,
) -> Element {
...
fn AddZoneForm(
    ...
    on_added: EventHandler<ExclusionZone>,
) -> Element {
...
#[component]
fn ZoneItem(zone: ExclusionZone, game_id: Uuid, on_removed: EventHandler<Uuid>) -> Element {
    let mut removing = use_signal(|| false);
    let zone_id = zone.id;

    let remove = move |_| {
        if *removing.read() { return; }
        removing.set(true);
        spawn(async move {
            if api::zones::remove_exclusion_zone(game_id, zone_id).await.is_ok() {
